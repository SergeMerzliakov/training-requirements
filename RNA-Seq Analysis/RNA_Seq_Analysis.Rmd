...
``` {r generateDES, echo=FALSE, message=TRUE}
...
# TODO move dds_res_df init this the generate block
...
ens2symbol <- AnnotationDbi::select(org.Hs.eg.db,
                                    keys=dds_res_df$ensgene, 
                                    columns="SYMBOL",
                                    keytype="ENSEMBL")

ens2symbol <- as_tibble(ens2symbol)
# add symbol to DEseqResults
dds_res_df <- inner_join(dds_res_df, ens2symbol, by=c("ensgene"="ENSEMBL"))

# symbol + test statistic. Take mean if multiple values for same symbols
dds_res_df2 <- dds_res_df %>% 
