...
LOW_EXPRESSION_THRESHOLD <- 10  # DEBUG - TODO CHANGE BACK to 10!!!!
...
"importance of accounting for the strikingly aberrant characteristics of HeLa cells when designing and interpreting experiments, and has implications for the use of HeLa as a model of human biology." Cervical cells expressibilities,
https://www.ncbi.nlm.nih.gov/pmc/articles/PMC3737162/


### Process
[Initial analysis](#Initial) reviewed the counts distribution and found DESeq2 criteria were satisfied.
...
The data was of a two-group experimental design with three replicates each of control (HeLa) and condition (Salmonella-infected HeLa).



## Principal Component Analysis
``` {r pcaData, echo=FALSE,warning=FALSE, message=FALSE}
...
```
A Principal Component Analysis (PCA) was performed on the data which showed that [first two components](#PCA) explain `r round(pca_sum$importance[3,2] * 100, 0)`% of the variance, and the [first four components](#PCA) approximately `r round(pca_sum$importance[3,4] * 100, 0)`%.

The component plots strong differentiation on the PC1 axis for the control and condition groups, with clustering for the control replicates, and Salmonella replicates split into two clusters. TODO why?

The interactions between the PC2 and PC3 are less marked, with clustering of the control replicates only. The remaining principle components provided no useful insights on the variance of Salmonella replicates.

``` {r pcaPlotImp, echo=FALSE, warning=FALSE, message=FALSE, fig.height=4, fig.width=10}
...
grid.arrange(pc_1and2, pc_2and3, ncol=2, nrow=1)
```
...
## Volcano Plot
The volcano plot TODO

``` {r volcano, warning=FALSE, message=FALSE, fig.height=4, fig.width=4}

dds_res_volcano <- data.frame(dds_res) %>%
              filter(abs(log2FoldChange) > L2FC_THRESHOLD, baseMean >= BASE_MEAN_THRESHOLD) %>%
              mutate(threshold = padj < P_THRESHOLD)   # strip off ensemble Id version

volcano <- ggplot(dds_res_volcano) +
...
## HeatMap
``` {r heatmapPost, fig.height=8, fig.width=6}
...
``` {r gsea, warning=FALSE, message=FALSE, fig.height=4, fig.width=8}
...
fgseaResTidy %>% 
  dplyr::select(-leadingEdge, -ES, -log2err, -pval, -size) %>% 
  arrange(padj) %>% 
  DT::datatable()
...
```{r assessDist,  warning=FALSE, message=FALSE,fig.height=4, fig.width=6, fig.align='center'}
...
The PCA analysis showed that the first 2 components contained the most useful explanatory information.
``` {r pcaAppx}
print(pca_sum)
...
grid.arrange(scree_plot, cumul_plot)
```
