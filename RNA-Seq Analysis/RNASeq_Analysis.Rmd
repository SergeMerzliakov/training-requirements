...
BiocManager::install(c("dplyr", "AnnotationDbi", "org.Hs.eg.db", "DESeq2", "tibble", "DT", "BiocParallel","ggplot2","EnhancedVolcano"))
...
# plot PCA by replicate
pca_df <- data.frame(replicate=rse_gene@colData$title, dds_pca$x)

varExp <- pca_sum$importance[2,] * 100
varDF = data.frame(Component=1:length(1:length(varExp)), varExp=varExp)

palette <- c("Mock_1" = "#f0b92e", 
             "Mock_2" = "#ff0000", 
             "Mock_3" = "#992020",
             "Salmonella_infected_1" = "#66c4fa",
             "Salmonella_infected_2" = "#1494de",
             "Salmonella_infected_3" = "#1414db")


pc_1and2 <- ggplot(pca_df) +
  geom_point(aes(x=PC1, y=PC2, color = replicate)) +
  xlab(paste0("PC1 (", round(varDF[1,2],1),"%)") ) +
  ylab(paste0("PC2 (", round(varDF[2,2],1),"%")) +
  scale_colour_manual(values = palette)

pc_2and3 <- ggplot(pca_df) +
  geom_point(aes(x=PC2, y=PC3, color = replicate)) +
...
dds_res <- lfcShrink(dds=dds, res=dds_res, type="apeglm", coef = 2)
...
dds_res_sig_overexp <- dds_res_df %>%
                       dplyr::filter(log2FoldChange > L2FC_THRESHOLD, 
...
sig_norm_counts <- norm_counts[dds_res_sig$GENEID,]
...
``` {r gsea,  fig.height=4, fig.width=8}
library(msigdbr)
library(clusterProfiler)

TOP_PATHWAYS <- 5
SIGNIFICANT_PATHWAYS_SAMPLE <- 100   # arbitrary choice
...
g <- ggplot(top_5_pathways, aes(reorder(ID, NES), NES)) +
  geom_col(aes(fill=p.adjust < P_THRESHOLD)) +
  coord_flip() +
  labs(x="Pathway",
...
``` {r gseaPlot, fig.height=5, fig.width=8, fig.align='center'}
...
gseaplot(res_gsea, geneSetID = "GOBP_WOUND_HEALING", title = "WOUND_HEALING")

gseaplot(res_gsea, geneSetID = "GOMF_CYTOKINE_ACTIVITY", title = "GOMF_CYTOKINE_ACTIVITY")

gseaplot(res_gsea, geneSetID = "GOMF_RECEPTOR_REGULATOR_ACTIVITY", title = "GOMF_RECEPTOR_REGULATOR_ACTIVITY")
...
The  top `r SIGNIFICANT_PATHWAYS_SAMPLE` pathways are shown in the table below. All have very close enrichment scores - from 1.88 for the first gene to 1.69 for the last pathway. So the 100 most significant pathways are too similarly expressed to make meaningful distinctions between them.
...
res_gsea_df %>%
  ...
  dplyr::select(ID, NES)  %>%
  arrange(desc(NES)) %>%
  slice_max(n = SIGNIFICANT_PATHWAYS_SAMPLE, order_by=NES, with_ties=F) %>%
  DT::datatable()
...
The PCA analysis showed that the first 2 components contained the most useful explanatory information. The plots below show the principal components.
``` {r pcaApp}
print(pca_sum)
...
cumul_plot <- ggplot(cumul_df, 
                    aes(x=c(1:nrow(cumul_df)), y=cumulativeProp)) + 
                    geom_point() +
...
grid.arrange(scree_plot, cumul_plot, ncol=2, nrow=1)
```
